...
#    print(f1)
    clas=f1[59:62]
#    print(clas)
# Get true label (Class label from folder name)                
    if (clas == 'asm'):
        Y = 0
    elif (clas == 'ben'):
        Y = 1
    ...
    Y=np.array([Y])  # True label to get the accuracy
...
path =  "/home/administrator/Muralikrishna_H/LID/IIITH/Shrikha/py_model/BLSTM1_2.pth"  # Test model path
...
folders = glob.glob('/home/administrator/Muralikrishna_H/LID/IIITH/test_BNF/*')  # Path of files to be tested
files_list = []
...
for fn in files_list:
    if i<1001:  # Take only 1000 samples, for getting early trend
